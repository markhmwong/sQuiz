...
-(id) init {
	if ((self = [super init])) {
        ...
        Level1NoTextSpriteSheet = [[CCTextureCache sharedTextureCache] addImage:@"Level1BGText.pvr"];
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Level1BGText.plist" texture:Level1NoTextSpriteSheet];
...
-(void) categoryMenu {
	...
	ActionButton = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"ActionText.png"] selectedSprite:nil target:target selector:@selector(LaunchLevel:)];
	...
	ComedyButton = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"ComedyText.png"] selectedSprite:nil target:target selector:@selector(LaunchLevel:)];
	...
	DramaButton = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"DramaText.png"] selectedSprite:nil target:target selector:@selector(LaunchLevel:)];
    ...
    statButtonSelected.color = ccc3(150, 150, 150);
    ...
    warning = [CCLabelTTF labelWithString:@"" fontName:@"OpenSans-Bold" fontSize:15.0 dimensions:CGSizeMake(200,150) hAlignment:kCCTextAlignmentCenter];
    warning.position = ccp(240, 140);
    warning.opacity = 0;
    [statLayer addChild:warning];
    
    scoreTitle = [CCLabelTTF labelWithString:@"Round Score" fontName:OPENSANSBOLD fontSize:15];
...
-(void) tick: (ccTime) dt
{
    ...
    if (categoryScroller.currentScreen == 0 && changeMusic) {
        ...
        [sae playBackgroundMusic:@"Action.aif"];
    }
    if (categoryScroller.currentScreen == 1 && changeMusic) {
        ...
        [sae rewindBackgroundMusic];
        [sae playBackgroundMusic:@"Comedy.aif"];
    }
    if (categoryScroller.currentScreen == 2 && changeMusic) {
        ...
        [sae rewindBackgroundMusic];
        [sae playBackgroundMusic:@"Drama.aif"];
    }
...
-(void) getStatsFromSettingsManager:(NSString *) category {
    ...
    BOOL categoryComplete = [statsManager getBool:[NSString stringWithFormat:@"%@ Complete", category]];
    int tempProgress = [statsManager getInt:[NSString stringWithFormat:@"%@ Progress", category]];
    if (categoryComplete) {
        [progressResult setString:[NSString stringWithFormat:@"%d / 100", tempProgress]];
        tempAnswered = 100;
    }
    else {
    ...
    //[self setRank:tempProgress totalProgressSoFar:tempAnswered];
    ...
    if (showReview) {
        ...
        switch (categoryScroller.currentScreen) {
            case 0:
                ...
                [categoryTitle setString:[NSString stringWithFormat:@"%@", CAT1]];
            ...
            case 1:
                ...
                [progressTitle runAction:[CCFadeIn actionWithDuration:1.0f]];
                [categoryTitle runAction:[CCFadeIn actionWithDuration:1.0f]];
                [categoryTitle setString:[NSString stringWithFormat:@"%@", CAT2]];
            ...
            case 2:
                ...
                [progressTitle runAction:[CCFadeIn actionWithDuration:1.0f]];
                [categoryTitle runAction:[CCFadeIn actionWithDuration:1.0f]];
                [categoryTitle setString:[NSString stringWithFormat:@"%@", CAT3]];
