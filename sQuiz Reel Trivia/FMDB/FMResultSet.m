...
- (BOOL) stringBoolForColumnIndex:(int)columnIdx {
	...
	//NSLog(@"stringBoolForColumnIndex: %i, %@", columnIdx, value);
	if ([value isEqualToString: @"Y"])  return YES;
	else return NO;
}

- (BOOL) stringBoolForColumn:(NSString*)columnName {
    return [self stringBoolForColumnIndex:[self columnIndexForName:columnName]];
}
